...
class MCLA_ProgressiveDecisionWorker(Process):
    """ MCLA Progressive Decision Worker """
    ...
    def run(self):
        ...
        while not np.all(runs_completed):
            ...
            mcla_labels = None
            mcla_labels = Cluster_Ensembles.mcla_ensemble(partitions, N_clusters_max=self.n_clusters, verbose=False)
            hgpa_n_clusters = len(np.lib.arraysetops.unique(mcla_labels))
            timestamp_after_decision = time.time()
            
            
            timestamp_before_inertia = time.time()
            hgpa_inertia = ClusteringMetrics.inertia(data, mcla_labels)
            ...
            ars = ClusteringMetrics.adjusted_rand_score(mcla_labels, previous_labels) if previous_labels is not None else 1
            ...
            ami = ClusteringMetrics.adjusted_mutual_info_score(mcla_labels, previous_labels) if previous_labels is not None else 1
            ...
            for i in range(self.n_runs):
                decision_ars[i] = ClusteringMetrics.adjusted_rand_score(partitions[i,:], mcla_labels)
                decision_ami[i] = ClusteringMetrics.adjusted_mutual_info_score(partitions[i,:], mcla_labels)
            ...
            result_labels = mcla_labels
            previous_labels = mcla_labels
            progressive_result = ProgressiveResult(result_info, result_labels)
