 ...
 *
 */
export default class PECWebSocketClient {
  constructor (url, verbose = false) {
    this.client = new JsonWebSocketClient(url, verbose)
    this.jobs = new Map()
  ...
  async getDatasetsInfo () {
    const list = await this.client.sendRequest('datasetsInfo')
    return list
  }

  async getDataset (name) {
    const d = await this.client.sendRequest(`dataset:${name}`)
    return d
  }
  ...
  async createAsyncJob (dataset, type, k, r, s) {
    ...
    const jobId = await this.client.sendRequest(`createAsyncJob:${JSON.stringify(req)}`)
  ...
  startJob (id) {
    this.client.sendMessage(`startJob:${id}`)
  }

  stopJob (id) {
    this.client.sendMessage(`stopJob:${id}`)
  }
}
/**
 *
